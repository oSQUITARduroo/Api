...
public class VehicleServiceImplTest {
  ...
  @Mock private VehicleDetailsRepository vehicleDetailsRepository;
  ...
  public void getVehicleDetailsSuccessFull() {
    ...
    List<VehicleDetails> actualVehicleDetailsList =
  ...
  public void getVehicleLocationNotFoundWhenVehicleDetailsAreNull() {
    VehicleDetails vehicleDetails = getDummyVehicleDetails();
    Mockito.when(vehicleDetailsRepository.findByUuid(vehicleDetails.getUuid())).thenReturn(null);
    VehicleLocationResponse vehicleLocationResponse =
        vehicleService.getVehicleLocation(vehicleDetails.getUuid());
  ...
  public void sendVehicleDetailsSuccessFullWithOwnerNull() {
    ...
    Mockito.when(userService.getUserFromToken(Mockito.any())).thenReturn(user);
    Mockito.when(userDetailsRepository.findByUser_id(Mockito.anyLong())).thenReturn(userDetails);
    Mockito.when(vehicleDetailsRepository.findAll()).thenReturn(vehicleDetailsList);
    Mockito.doNothing()
        .when(smtpMailServer)
    ...
    Assertions.assertEquals(
        UserMessage.VEHICLE_DETAILS_SENT_TO_EMAIL, crapiAPIResponse.getMessage());
    Assertions.assertEquals(HttpStatus.OK.value(), crapiAPIResponse.getStatus());
  }
